...
export default function PropertyCard({
  ...
  const formatIQD = (val: number) => {
    if (val >= 1) {
      ...
      // E.g. 0.55 Million = 550,050 Dinars
  ...
  return (
    <article
      ...
      className="group relative flex flex-col bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl hover:border-emerald-600/20 transition-all duration-300 overflow-hidden cursor-pointer h-full"
    >
      
      {/* Top Visual Slider */}
      <div id="image-slider" className="relative aspect-video w-full overflow-hidden bg-slate-100">
        <img
      ...
      <div className="flex-1 flex flex-col p-4">
        ...
        <div className="flex items-center gap-4 mt-3 py-2 border-y border-slate-100 text-slate-600 text-xs">
        ...
        <div id="pricing-container" className="mt-4 flex flex-col gap-0.5 text-right font-sans">
          <div className="flex items-baseline justify-between">
            <span className="text-xs text-slate-500">القيمة بالدينار:</span>
          ...
          {(!property.priceIQD && property.priceUSD > 0) && (
            <div className="text-[9px] text-emerald-800 text-right mt-1.5 font-medium bg-emerald-50 px-2.5 py-1 rounded-lg">
              * سعر صرف تقريبي متوقع بالسماوة (100$ = 150,000 د.ع)
            </div>
          )}
        </div>
      ...
      {/* Buttons Actions Layer */}
      <div id="action-buttons-layer" className="grid grid-cols-2 bg-slate-50 border-t border-slate-100/60 p-2 gap-2">
        <button
          onClick={(e) => onCall(property.broker.phone, e)}
          className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold text-white bg-emerald-800 hover:bg-emerald-950 hover:shadow-md transition-all shrink-0"
        >
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold text-white bg-green-600 hover:bg-green-700 hover:shadow-md transition-all shrink-0"
        >
